        ...
        Form {
            ...
            .alert(isPresented: $showAlert) {
                Alert(title: Text("Error"),
                      message: Text("The product could not be updated"),
                      dismissButton: .default(Text("Dismiss")))
            }
        }
        .onAppear(perform: {
            self.editProductViewModel.setProduct(product: self.product)
        })
    }
}
...
struct EditProductView_Previews: PreviewProvider {
    static var previews: some View {
        EditProductView(product: Product(id: 0, name: "product", description: "desc", code: "cod", price: 10.0)) { product in
